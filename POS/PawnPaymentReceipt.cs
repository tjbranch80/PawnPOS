...
namespace POS
{
    public partial class PawnPaymentReceipt : Form
    {
        #region Global Components

        string connectionString;
        
        public PawnPaymentReceipt()
        {
            InitializeComponent();
            CenterToScreen();
        ...
        #region Pawn Payment Data Collection
        ...
        public string TransactionNumber
        {
            ...
            set
            {
                ...
                pawnPaymentInfoTable = dataManager.GetPawnPaymentReceiptData(transactionNumber);

                DateTime paymentDate = DateTime.Now;
                PaymentDateLabel.Text = paymentDate.ToShortDateString();

                DataTable currentPrincipalTable = new DataTable();
                currentPrincipalTable = dataManager.GetPawnCurrentPrincipal(transactionNumber);
                
                var principal = currentPrincipalTable.Rows[0]["CurrentPrincipal"];
                int currentPrincipal = Convert.ToInt32(principal);
                NewPrincipalLabel.Text = string.Format("{0:C}",currentPrincipal);
        ...
        public string PaymentAmount
        {
            ...
            {
                paymentAmount = value;
                int convertedPaymentAmount = Convert.ToInt32(paymentAmount);
                PaymentAmountLabel.Text = string.Format("{0:C}",convertedPaymentAmount);
            }
        }
        ...
        {
            var pawnPaymentReceiptPath = ConfigurationManager.AppSettings["InvoicePath"];
            string transactionID = "PP" + transactionNumber;
            string savePath = pawnPaymentReceiptPath + transactionID + ".jpg";
